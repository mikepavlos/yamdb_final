...
Запуск проекта на сервере котандой

```commandline
sudo docker-compose up -d
```

Либо при `push` на GitHub, проект тестируется, обновляется его образ и автоматически разворачивается на сервере.  

После успешного развертывания пректа приходит сообщение на telegram.  
...
Документация к проекту с подробными примерами запросов и ответов к endpoints проекта доступны после запуска проекта по адресу:

http://<ip_адрес_хоста>/redoc
